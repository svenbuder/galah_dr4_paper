...
In order to confront these model spectra with the GALAH observations, we downgrade the model spectra on-the-fly with the available line-spread functions of each observation (see Sec.~(subsec:lsf)).
...
\SB{We mask pixels that are within predefined masks from solar spectrum comparison}
\SB{We also mask pixels that differ more than 0.25 of the normalised flux and 5 sigma of it}
